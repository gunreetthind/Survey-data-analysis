...
paper <- read_excel("C:/Users/gunreet.thind/Documents/FCC/Agweek Survey.xlsx", sheet = 1) %>% as_tibble()

online <- read_excel("C:/Users/gunreet.thind/Documents/FCC/Agweek Survey.xlsx", sheet = 2) %>% as_tibble()
...
#86% of surveyed people don't grow winter wheat so FCC may not print content around it as it may not be relevant to the target audience
...
#Trying to use ggplotly, but throwing error, so parked it here for now
...
##############Should I think on these lines more?############      
...
# Observation: Target audience reads content around ag news, news markets, farm news, agricultural info, so providing specialised knowledge, latest ag developments should be FCC's focus
```
...
# Observation: Fewer people watch agweek TV as compared to agweek magazine. FCC can work to improve communication through this channel
```
```{r}
...
p3 <- ggplot(top_10) +
  ...
  ylab("Number of Occurrences")
```
Observation: FCC can focus on more future stories around ryan taylor, pinke post,shawna olson etc.
```{r}
...
Observation: Local stories or localisation of content could be of key importance for FCC, followed by more Ads,auction sales and market news respectively
```{r}
...
all_zip <- read_excel("C:/Users/gunreet.thind/Documents/FCC/AGWEEK list by zip.xlsx", sheet = 1) %>% as_tibble()

all_zip <- all_zip%>% filter(AGWEEK!="NA")

# zip_sample <- distinct(survey,zip)
# 
# zip_pop <- distinct(all_zip,ZIPCODE)


colnames(all_zip)[1] <- "zip"

all_zip$zip <- as.numeric(all_zip$zip)

survey_1 <- survey%>% left_join(y = all_zip,by="zip")

survey_2 <- survey_1%>% filter(zip!="NA")%>%
            group_by(zip)%>%
            mutate(count=n())%>%
            mutate(weight=round(count/AGWEEK*100,2))

write.csv(survey_2,"sample.csv")

x <- survey_2%>%filter(weight==200)  #data quality issues

y <- survey_2%>%filter(zip==59215)
